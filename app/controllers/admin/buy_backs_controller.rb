class Admin::BuyBacksController < ApplicationController
      ...
      format.js{
        if params[:delivery_id]
          render :update do |page|
            page.replace_html(:delivery_id_container, "##{@delivery.id}")
  ...
  def create
    ...
    if @buy_back.save
      success "Successfully created buyback."
      redirect_to [:admin, @buy_back]
    else
      render :action => 'new'
    end
  end
  ...
  def update
    ...
    if @buy_back.update_attributes(params[:buy_back])
      success "Successfully updated buyback."
      redirect_to [:admin, @buy_back]
    else
      render :action => 'edit'
    end
  ...
  def destroy
    @buy_back = BuyBack.find(params[:id])
    @buy_back.destroy
    success "Successfully destroyed buyback."
