class Delivery < ActiveRecord::Base
  ...
  def donut_count
    line_items.map{|i| i.quantity if i.donut? }.compact.map(&:sum)
  end
  
  def roll_count
    line_items.map{|i| i.quantity if i.roll? }.compact.map(&:sum)
  end
  
  def donut_hole_count
    line_items.map{|i| i.quantity if i.donut_hole? }.compact.map(&:sum)
  end
